...
var stockHelper = {
  ...
  getDetailDataUrl: function getDetailDataUrl() {
    ...
    return "https://iss.moex.com/iss/engines/stock/markets/shares/boards/TQBR/securities/MTSS.json?iss.meta=off&iss.json=extended&iss.only=marketdata,securities&marketdata.columns=SECID,".concat(last, ",").concat(change, ",").concat(lastToPrevPrice, ",").concat(issueCapitalization, ",").concat(volToday, ",").concat(high, ",").concat(low, ",").concat(open, "&securities.columns=PREVDATE,").concat(prevLegalClosePrice);
  },
  getChartDataUrl: function getChartDataUrl(startDate, endDate) {
    var start = this.startPage * this.pageSize;
    return "https://iss.moex.com/iss/history/engines/stock/markets/shares/sessions/total/boards/TQBR/securities/MTSS.json?iss.meta=off&iss.json=extended&history.columns=TRADEDATE,LEGALCLOSEPRICE&sort_order=desc&from=".concat(startDate, "&till=").concat(endDate, "&start=").concat(start);
  },
  startPage: 0,
  pageSize: 100,
  ...
  chartDefaultSettings: {
    ...
    chart: {
      style: {
        fontFamily: "MTSSans, Helvetica, Arial, sans-serif"
      },
